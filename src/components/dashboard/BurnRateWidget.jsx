...
export const BurnRateWidget = ({ 
  ...
  const getStatusColor = () => {
    if (!hasSpendingData) return 'text-slate-500 border-slate-200 bg-slate-100'
  ...
  const getRunwayColor = () => {
    if (!isSafe) return 'bg-gradient-to-r from-red-500 to-rose-500'
    if (projectedRunwayDays < daysRemaining * 0.5) return 'bg-gradient-to-r from-amber-400 to-orange-500'
  ...
  return (
    <div className={`bg-white rounded-2xl shadow-md border p-5 md:p-6 relative overflow-hidden transition-all duration-300 ${
      hasSpendingData ? (isSafe ? 'border-slate-100 shadow-slate-100/40' : 'border-red-100 shadow-red-50/30') : 'border-slate-100 bg-slate-50/30'
    }`}>
      {/* Top Accent Strip */}
      <div className={`absolute top-0 inset-x-0 h-1 ${!hasSpendingData ? 'bg-slate-200' : isSafe ? 'bg-emerald-500' : 'bg-red-500'}`} />
      ...
      <div className="flex justify-between items-start mb-4">
        <div className="flex flex-col items-start gap-3">
          ...
          {/* Styled Native Dropdown (Dynamic Width & Stacked) */}
        ...
        <div className={`px-2.5 py-1.5 rounded-lg text-[10px] font-bold tracking-wider flex items-center gap-1.5 border shadow-sm ${getStatusColor()}`}>
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm text-center">
          <p className="text-[9px] text-slate-400 uppercase font-bold tracking-wider mb-1">Balance</p>
          <p className={`text-sm font-black ${currentBalance < 100 ? 'text-red-600' : 'text-slate-800'}`}>
            {formatMYR(currentBalance)}
          </p>
        </div>
        <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm text-center">
          <p className="text-[9px] text-slate-400 uppercase font-bold tracking-wider mb-1">Daily Avg</p>
          <p className="text-sm font-black text-slate-800">
            {hasSpendingData ? formatMYR(averageDailySpend) : '—'}
          </p>
        </div>
        <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm text-center">
          <p className="text-[9px] text-slate-400 uppercase font-bold tracking-wider mb-1">Runway</p>
          <p className={`text-sm font-black ${!hasSpendingData ? 'text-slate-400' : projectedRunwayDays < 7 ? 'text-red-600' : 'text-slate-800'}`}>
            {isInfiniteRunway ? '∞' : `${projectedRunwayDays}d`}
          </p>
        </div>
      </div>

      {/* Progress Metric Bars (Only render if there is data) */}
      {hasSpendingData ? (
        <div className="space-y-4 mb-2">
          ...
          <div>
            <div className="flex justify-between text-xs mb-2">
              <span className="text-slate-500 font-medium flex items-center gap-1.5">
                <Zap className="w-3.5 h-3.5 text-amber-500" /> Projected Runway
              </span>
              <span className="font-bold text-slate-700 text-xs">
                {isInfiniteRunway ? '∞' : `${projectedRunwayDays}d`} / {daysRemaining}d left
              </span>
            </div>
            <div className="w-full bg-slate-100 rounded-full h-2 overflow-hidden shadow-inner">
              <div 
        ...
        <div className="flex flex-col items-center justify-center p-6 bg-white/50 border-2 border-slate-200 border-dashed rounded-2xl mt-2 animate-fadeIn">
          ...
          <p className="text-xs text-slate-400 mt-1 text-center max-w-[200px] leading-relaxed">
            Log an expense from this account to activate runway calculations.
          </p>
        </div>
      )}
      ...
      {hasSpendingData && (
        <button
          ...
          className="w-full flex items-center justify-center gap-1.5 mt-5 pt-3 border-t border-slate-100/80 text-xs font-semibold text-slate-400 hover:text-slate-600 transition-colors group"
        >
        ...
        <div className={`grid transition-all duration-300 ease-in-out ${showDetails ? 'grid-rows-[1fr] opacity-100 mt-4' : 'grid-rows-[0fr] opacity-0'}`}>
          <div className="overflow-hidden space-y-4">
            ...
            <div className="bg-slate-50/60 p-3.5 rounded-xl border border-slate-100/80 flex justify-between items-center gap-4">
              ...
              <div className={`px-2.5 py-1 rounded-md text-[10px] font-bold tracking-wider shrink-0 border shadow-sm ${
